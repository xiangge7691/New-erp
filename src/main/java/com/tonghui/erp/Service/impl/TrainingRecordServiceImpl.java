...
public class TrainingRecordServiceImpl extends ServiceImpl<TrainingRecordMapper, TrainingRecord>
    ...
    public boolean addTrainingRecord(TrainingRecord trainingRecord) {
        ...
        boolean autoGenerated = !StringUtils.hasText(trainingRecord.getTrainingNo());
        if (autoGenerated) {
            // 自动生成编号已统计包含软删除记录的最大序号，保证不与任何记录（含软删除）冲突
        ...
        while (true) {
            try {
                ...
                if (!autoGenerated || ++attempts >= 3) {
                    throw new RuntimeException("创建失败: 培训编号生成冲突，请稍后重试", e);
                }
                trainingRecord.setTrainingNo(generateTrainingNo());
            }
        }
