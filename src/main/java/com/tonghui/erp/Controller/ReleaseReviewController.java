...
public class ReleaseReviewController extends BaseController {
     ...
     */
    @Autowired
    private com.tonghui.erp.Data.mapper.WorkOrderMapper workOrderMapper;
     ...
     * 查询所有已检验状态的工单列表
     * <p>
     * 用于审核放行新增/编辑时关联生产任务下拉选择，
     * 仅返回inspection_end不为空（状态为"已检验"）的工单
     * </p>
     *
     * 示例请求：
     * GET /api/releaseReview/workOrders
     *
     * @return ApiResponse&lt;List&lt;WorkOrder&gt;&gt; 已检验工单列表
     */
    @GetMapping("/workOrders")
    public ApiResponse<List<com.tonghui.erp.Data.Entity.WorkOrder>> getInspectedWorkOrders() {
        try {
            List<com.tonghui.erp.Data.Entity.WorkOrder> list = workOrderMapper.selectInspectedOrders();
            return success(list);
        } catch (Exception e) {
            return exception(e, "查询已检验工单");
        }
    }

    /**
     * 根据工单ID查询成品且合格的检验记录列表
     * <p>
     * 用于审核放行新增/编辑时，选中工单后自动带出对应的检验编号，
     * 筛选条件：item_category=成品 且 conclusion=合格
     * </p>
     *
     * 示例请求：
     * GET /api/releaseReview/inspection/1
     *
     * @param workOrderId 工单ID（路径参数）
     * @return ApiResponse&lt;List&lt;InspectionRecord&gt;&gt; 成品且合格的检验记录列表
     */
    @GetMapping("/inspection/{workOrderId}")
    public ApiResponse<List<InspectionRecord>> getQualifiedInspections(@PathVariable Long workOrderId) {
        try {
            List<InspectionRecord> list = inspectionRecordMapper.selectQualifiedFinishedProducts(workOrderId);
            return success(list);
        } catch (Exception e) {
            return exception(e, "查询成品合格检验记录");
        }
    }
    ...
    public ApiResponse<ReleaseReview> create(@RequestBody ReleaseReview record) {
        try {
            ...
            if (record.getWorkOrderId() != null) {
                com.tonghui.erp.Data.Entity.WorkOrder workOrder = workOrderService.getById(record.getWorkOrderId());
                if (workOrder == null) {
                    return error("关联生产任务不存在");
                }
                if (!"已检验".equals(workOrder.getCurrentStatus())) {
                    return error("关联生产任务必须是已检验状态，当前状态：" + workOrder.getCurrentStatus());
                }
                // 自动带出成品且合格的检验编号（若未手动传入）
                if (!StringUtils.hasText(record.getRelatedInspectionCode())) {
                    List<InspectionRecord> inspRecords = inspectionRecordMapper.selectQualifiedFinishedProducts(record.getWorkOrderId());
                    if (!inspRecords.isEmpty()) {
                        record.setRelatedInspectionCode(inspRecords.get(0).getInspectionCode());
                    }
                }
            }
